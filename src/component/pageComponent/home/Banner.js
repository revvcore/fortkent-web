...
import { videoData } from '@/data/bannerData';
...
export default function Banner() {
  ...
  const [currentVideosPerPage, setCurrentVideosPerPage] = useState(videosPerPage.desktop);
    ...
    if (isAutoScrolling) {
      autoScrollRef.current = setInterval(() => {
        setCurrentIndex((prevIndex) => {
          const maxIndex = Math.ceil(videoData.length / currentVideosPerPage) - 1;
  ...
  const getCurrentVideos = () => {
    ...
    return videoData.slice(startIndex, startIndex + currentVideosPerPage);
  };

  const totalPages = Math.ceil(videoData.length / currentVideosPerPage);
